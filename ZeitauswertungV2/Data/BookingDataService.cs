...
namespace ZeitauswertungV2.UI.Data
{
...

    {
        ...
        public async Task<List<Booking>> GetByEmployeeIdAsync(string employeeId)
        {
            using (var ctx = contextCreator())
            {
                return await ctx.Bookings.AsNoTracking().Where(b=>b.Employee ==employeeId).Where(b=>b.Deleted!=true).OrderByDescending(b => b.Date).ToListAsync();               
            }
        }

        public List<Booking> GetByEmployeeIdAndDate(string employeeId, DateTime from, DateTime till )
        {
            if (till < from)
            {
                till = DateTime.Now;
            ...
            using (var ctx = contextCreator())
            {
                return ctx.Bookings.AsNoTracking().Where(b => b.Employee == employeeId).Where(b=>b.Date>=from&&b.Date<=till).Where(b => b.Deleted != true).OrderByDescending(b=>b.Date).ToList();
            }
        }
    }
}
